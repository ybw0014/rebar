    ...
    override fun run() {
        for (player in Bukkit.getOnlinePlayers()) {
            for (item in player.inventory) {
                val rebarItem = RebarItem.fromStack(item, RebarInventoryTickerItem::class.java)
                if (rebarItem is RebarInventoryTickerItem && count % rebarItem.baseTickInterval == 0L) {
